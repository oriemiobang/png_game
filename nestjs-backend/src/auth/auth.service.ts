...
export class AuthService {
  ...
  async getMyStats(userId: string) {
    ...
    const matchHistory = recentGames.map(game => {
      ...
      let outcome = 'draw';
      if (game.winnerId === userId) outcome = 'win';
      else if (game.winnerId && game.winnerId !== userId) outcome = 'loss';
  ...
  async getLeaderboard() {
    ...
    return users.map((u) => {
      ...
      let tier = 'Beginner';
      if (u.rating >= 2200) tier = 'Master';
      else if (u.rating >= 1800) tier = 'Expert';
      else if (u.rating >= 1400) tier = 'Advanced';
      else if (u.rating >= 1000) tier = 'Intermediate';
